...
static CFont   _font;
CFont TFXDataTip::m_FontValue, TFXDataTip::m_FontName;
...
static LPCTSTR	szClassName = _T("TFXDataTip");


BEGIN_MESSAGE_MAP(TFXDataTip, ParentClass)
...
void TFXDataTip::Initialise( )
{
    if (_font.GetSafeHandle( ) == NULL)
    {
		...
		m_FontName.CreateFontIndirect(&lFont);
		lFont.lfWeight = FW_BOLD;
		m_FontValue.CreateFontIndirect(&lFont);
...
BOOL TFXDataTip::Create(CWnd* pParentWnd) 
{
	...
	m_ready = CWnd::CreateEx(WS_EX_LEFT | WS_EX_TOOLWINDOW,
                             ...
                             pParentWnd->m_hWnd, 
                             NULL);
//	m_ready = CToolTipCtrl::Create( pParentWnd, WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP );
...
void TFXDataTip::Display( )
{
	...
	nmhdr.code		= NM_USER_PROPERTY;
	nmhdr.idFrom	= GetDlgCtrlID();

	CWnd *pWnd = m_parent;
	LRESULT res = pWnd->SendMessage(WM_NOTIFY, (WPARAM)GetSafeHwnd(), (LPARAM)&nmhdr);
	if(!res) 
	{
//		_current->Hide( );
		return;
...
{
    KillTimer(m_timer);
	return ParentClass::DestroyWindow();
}
...
//	CSize size = dc.GetTextExtent(m_tip);
	...
	for(i=0; i < m_Property.size();++i)
	{
		CString str = m_Property[i].first;
		CSize sizeItem = dc.GetTextExtent(str);
		dwOffset = max( dwOffset, sizeItem.cx );
	}

	dc.SelectObject(&m_FontValue);
	for( i=0; i < m_Property.size();++i)
	{
		CString str = m_Property[i].second;
		CSize sizeItem = dc.GetTextExtent(str);
		dwOffset2 = max( dwOffset2, sizeItem.cx );
	}

	CRect rcItem1(_border, 0, dwOffset + _border, _border);
	CRect rcItem2(dwOffset + c_nGap + _border, 0, dwOffset2 + dwOffset + c_nGap + _border, _border);
	for( i = 0; i < m_Property.size(); ++i )
	{
		
		dc.SetTextColor( RGB(0x42,0,0xC0) );
		CString str = m_Property[i].first;
		CSize sizeItem = dc.GetTextExtent(str);
		
		rcItem1.bottom = rcItem1.top + sizeItem.cy;
		dc.SelectObject(&m_FontName);
		dc.DrawText( str, -1, &rcItem1, DT_SINGLELINE|DT_LEFT|DT_TOP/*|DT_END_ELLIPSIS*/ );
		rcItem1.top = rcItem1.bottom;

		str = m_Property[i].second;
		sizeItem = dc.GetTextExtent(str);
		
		dc.SetTextColor( RGB(0,0x80,0x80) );
		rcItem2.bottom = rcItem2.top + sizeItem.cy;
		dc.SelectObject(&m_FontValue);
		dc.DrawText( str, -1, &rcItem2, DT_SINGLELINE|DT_LEFT|DT_TOP/*|DT_END_ELLIPSIS*/ );
		rcItem2.top = rcItem2.bottom;
	}
...
{
	ParentClass::OnMouseMove(nFlags, point);
...
CSize	TFXDataTip::GetSize()
{
    ...
    CClientDC dc(this);
    CFont* pOld = (CFont*)dc.SelectObject(&m_FontName);
	int i;
//	CSize size = dc.GetTextExtent(m_tip);
	...
	for( i=0; i < m_Property.size();++i)
	{
		...
		CSize sizeItem = dc.GetTextExtent(str);
		size.cy += sizeItem.cy;
		size.cx = max( size.cx, sizeItem.cx );
	}
	dc.SelectObject(&m_FontValue);
	DWORD dwOffset = 0;
	for( i=0; i < m_Property.size();++i)
	...
	size.cx += dwOffset > 0?  dwOffset + c_nGap : 0;
...
void	TFXDataTip::OnLButtonDown( UINT nFlags, CPoint point )
{
	...
	m_parent->SendMessage( WM_LBUTTONDOWN, nFlags, pt.y << 16| pt.x);
}
...
void	TFXDataTip::OnLButtonUp( UINT nFlags, CPoint point )
{
	...
	ClientToScreen(&pt);
	m_parent->ScreenToClient(&pt);
	m_parent->SendMessage( WM_LBUTTONUP, nFlags, pt.y << 16| pt.x);
}
...
void	TFXDataTip::OnRButtonDown( UINT nFlags, CPoint point )
{
	...
	ClientToScreen(&pt);
	m_parent->ScreenToClient(&pt);
	m_parent->SendMessage( WM_RBUTTONDOWN, nFlags, pt.y << 16| pt.x);
}
...
void	TFXDataTip::OnRButtonUp( UINT nFlags, CPoint point )
{
	...
	ClientToScreen(&pt);
	m_parent->ScreenToClient(&pt);
	m_parent->SendMessage( WM_RBUTTONUP, nFlags, pt.y << 16| pt.x);
}
