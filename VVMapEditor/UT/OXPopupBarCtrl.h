...
// the message that floating popup bar sends to its parent window when 
// the current selected element has been changed
#define OXPBN_SELCHANGED			WM_APP+326
...
public:
	...
	//														be displayed
	//								POPUPBAR_NOFLAT		-	reserved for future use
    //								POPUPBAR_NOTEAROFF  -   "Tear-off" available
	// --- Out : 
	...
	// --- Returns:	
	// --- Effect : set selected button using data associated with it
	inline void SetSelectedData(const DWORD dwSelected) { m_dwSelected=dwSelected; }
	...
	// --- Returns:	
	// --- Effect : set the data of default button
	inline void SetDefaultData(const DWORD dwDefault) { m_dwDefault=dwDefault; }
	...
	// --- Returns:	
	// --- Effect : set common button size
	inline void SetButtonSize(const CSize sizeButton) { m_sizeButton=sizeButton; }
