...
/*		LOGFONT lf;
		if(pBaseCalendar->GetFont()->GetLogFont(&lf))
...
BOOL  COXCalendarPopup::Pick
		(COleDateTime& pickDate,
	...
	for(;;)
		...
		switch (msg.message)
			...
			case WM_ENDPICK:
				...
				bPicked = msg.wParam;// if TRUE, date has been selected,
...
void COXCalendarPopup::OnLButtonDown(UINT /* nFlags */, CPoint point) 
	...
	if(nHitTest != -1)
		...
		if(m_InitialSelectedDate != m_DatePicked)
			{
...
void COXCalendarPopup::OnPaint() 
	...
	for(i=0;i < 7;i++)
		{
//		CString sWeekDay = COXCalendar::ms_straWeekDays[(i + m_nFirstDayOfWeek)%7][0];
		///
		CString sWeekDay = COXCalendar::ms_straWeekDays[(i + m_nFirstDayOfWeek)%7];
