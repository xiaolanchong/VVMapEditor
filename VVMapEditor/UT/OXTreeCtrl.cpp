	...
	case  ODA_DRAWENTIRE:
		...
		if(m_bCreatingDragImage)
		{
			...
			if(dc.RectVisible(rcItem))
			{
				...
				clrForground = dc.SetTextColor(xti->IsDisabled() ? 
					::GetSysColor(COLOR_3DSHADOW) : xti->HasColor(nCol) ?
					xti->GetItemColor(nCol) : GetTextColor());
				clrBackground = dc.SetBkColor((!IsWindowEnabled() ? 
				...
				// draw item's text 
				DrawItemText(&dc,sTxt,rcItem,lvc.fmt,FALSE,(!IsWindowEnabled() ? 
					::GetSysColor(COLOR_BTNFACE) : GetTextBkColor()),nEllipsisFormat);
				dc.SetTextColor(clrForground);
		...
		else
		{
			...
			for(nCol = 0; nCol < nColCnt;nCol++)
				...
				if(dc.RectVisible(rcItem))
					{
					DrawItemImages(&dc,xti,itemID,rcItem,nCol,bRowSel && bShowItemSel);

					if(nCol == 0)
						nFocusRectOffset = rcItem.left;
						
					clrForground = dc.SetTextColor(bShowItemSel && (nCol == 0 || bRowSel) ?
						::GetSysColor(COLOR_HIGHLIGHTTEXT) : xti->IsDisabled() ? 
						::GetSysColor(COLOR_3DSHADOW) : xti->HasColor(nCol) ?
						xti->GetItemColor(nCol) : GetTextColor());
					clrBackground = dc.SetBkColor(bShowItemSel && 
						(nCol == 0 || bRowSel) ?
						::GetSysColor(COLOR_HIGHLIGHT) : (!IsWindowEnabled() ? 
						::GetSysColor(COLOR_BTNFACE) : GetTextBkColor()));
					...
					DrawItemText(&dc,sTxt,rcItem,lvc.fmt,
						(lpDIS->itemState & ODS_FOCUS) && bFocus && nCol == 0 && !bRowSel, 
						bRowSel && bShowItemSel ? ::GetSysColor(COLOR_HIGHLIGHT) : 
...
/*			CRect rcEdit;
			...
			if(nCol!=-1)
			{
				point=rcEdit.BottomRight()-CPoint(2,2);
				MSG& msg=AfxGetThreadState()->m_lastSentMsg;
				// save original wParam
				WPARAM oldLP=msg.lParam;	
...
HTREEITEM COXTreeCtrl::InsertItem( LPCTSTR lpszItem, HTREEITEM hParent, HTREEITEM hInsertAfter)
{
	if(hParent==NULL)
		hParent=TVI_ROOT;
...
int COXTreeCtrl::GetItemIndex(HTREEITEM hItem)
{
	return GetItemIndexInternal(GetXItem(hItem));
}

int COXTreeCtrl::GetItemIndexInternal(COXTreeItem *pItem)
{
	if(pItem == &m_xtiRoot)
		return -1;
	LV_FINDINFO lvfi;
...
COXTreeItem* COXTreeCtrl::GetXItem(HTREEITEM hti)
{
...
void COXTreeCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if(idx != -1 && flgs & TVHT_ONITEM)
	{
		Expand((HTREEITEM)CListCtrl::GetItemData(idx),TVE_TOGGLE);
		CListCtrl::RedrawItems(idx,idx);
	}
	else if(idx != -1 && (flgs&TVHT_ONITEMBUTTON || 
		(GetPickAnywhere() && (flgs&TVHT_NOWHERE)!=TVHT_NOWHERE)))
	{
		Expand((HTREEITEM)CListCtrl::GetItemData(idx),TVE_TOGGLE);
		CListCtrl::RedrawItems(idx,idx);
	}
...
/*	int nInsertPoint;
	...
	static TCHAR szTChar[_OX_MAX_ITEM_TEXT];
	static CSize sz;
	static CSize DotsTextWidth = pDC->GetTextExtent(_T("..."), 3);
// calculate text rect; draw dots if text extent more than column width
	int nColWidth = rcText.right - rcText.left - 4;
	_tcscpy(szTChar, sText);
	int len = _tcslen(sText);
	sz = pDC->GetTextExtent(szTChar, len);
	if (nColWidth < sz.cx)
		{
		while (nColWidth < (sz.cx + DotsTextWidth.cx) && sz.cx)
			{
			szTChar[--len] = _T('\x0');
			sz = pDC->GetTextExtent(szTChar, len);
			}
		...
		_tcscat(szTChar, _T("..."));
		sz.cx += DotsTextWidth.cx;
		len += 3;
		}
	if(sz.cx < rcText.Width())
		{
		COLORREF clrBack = pDC->GetBkColor();
		switch(nJustify & LVCFMT_JUSTIFYMASK)
			{
			case LVCFMT_LEFT:
				rcText.right = __min(rcText.left + sz.cx + 4,rcTxt.right);
				rcFill.left = rcText.right;
				// clear the right side of text
				if(!rcFill.IsRectEmpty())
					pDC->FillSolidRect(rcFill,clrFillOutside);
				break;
			case LVCFMT_RIGHT:
				// clear the left side of text
				rcText.left = __max(rcText.right - sz.cx - 4,rcTxt.left);
				rcFill.right = rcText.left;
				if(!rcFill.IsRectEmpty())
					pDC->FillSolidRect(rcFill,clrFillOutside);
				break;
			case LVCFMT_CENTER:
				int nCenter = rcText.left + rcText.Width()/2;
				rcText.left = nCenter - sz.cx/2 - 2;
				// clear the left side of text
				rcFill.right = rcText.left;
				if(!rcFill.IsRectEmpty())
					pDC->FillSolidRect(rcFill,clrFillOutside);
				rcText.right = nCenter + sz.cx/2 + 2;
				rcFill.left = rcText.right;
	...
	switch(nJustify & LVCFMT_JUSTIFYMASK)
		...
		case LVCFMT_LEFT:
			...
			nInsertPoint = rcText.left + 2;
			break;
		case LVCFMT_RIGHT:
			pDC->SetTextAlign(TA_RIGHT);
			nInsertPoint = rcText.right - 2;
			break;
		case LVCFMT_CENTER:
			pDC->SetTextAlign(TA_CENTER);
			nInsertPoint = rcText.left + ((rcText.right - rcText.left)/2);
			break;
		default:
			ASSERT(FALSE);
			break;
	...
	// Draw the text
	pDC->ExtTextOut(nInsertPoint, rcText.top+1,
					ETO_OPAQUE | ETO_CLIPPED, rcText,
					szTChar,len,NULL);
...
BOOL COXTreeCtrl::GetShowSelAlways()
{
...
BOOL COXTreeCtrl::HasButtons()
{
	return (m_dwTCStyle & TVS_HASBUTTONS);
}

BOOL COXTreeCtrl::HasLines()
{
	return (m_dwTCStyle & TVS_HASLINES);
}

BOOL COXTreeCtrl::HasLinesAtRoot()
{
	return (m_dwTCStyle & TVS_LINESATROOT);
}


int COXTreeCtrl::GetItemStateIndex(int nItem)
{
	LV_ITEM lvI;
...
void COXTreeCtrl::DrawLine(CDC *pDC,CPen* pPen,CRect& rc,int nMode)
{
	...
	case 3:
		...
		break;
	case 4:
		pDC->MoveTo(rc.left + rc.Width()/2,rc.bottom);
		pDC->LineTo(rc.left + rc.Width()/2,rc.top + rc.Height()/2);
		pDC->LineTo(rc.right,rc.top + rc.Height()/2);
		break;
	default:
...
int COXTreeCtrl::HitTest( CPoint pt, UINT* pFlags)
{
	UINT nListFlags;
...
int COXTreeCtrl::GetItemIndent(COXTreeItem *pItem)
{
...
void COXTreeCtrl::GetItemPartRect(COXTreeCtrl::eItemPartRect eType,COXTreeItem *pItem,CRect *rTarget,int *nIndent)
{
...
CSize COXTreeCtrl::GetColTextExtent(int nIndex,int nCol)
	// --- In  :
	// --- Out : 
...
int COXTreeCtrl::GetColumnsCount()
{
	int cnt = 0;
	LV_COLUMN lvc;
	memset(&lvc, 0, sizeof(LV_COLUMN));
...
void COXTreeCtrl::DrawItemImages(CDC *pDC,COXTreeItem* xti,int nItem,CRect& rcItem,int nCol,BOOL bRowSel)
{
	...
	else
	{
		...
		if(nImg >= 0)
		{
			...
			if(bRowSel)
			{
				clrBack = pImgList->SetBkColor(::GetSysColor(COLOR_HIGHLIGHT));
				COLORREF clrBkDC = pDC->GetBkColor();
				pDC->FillSolidRect(&rcErase,::GetSysColor(COLOR_HIGHLIGHT));
...
BOOL COXTreeCtrl::GetItem(TV_ITEM *pTVI)
{
	COXTreeItem *xti = GetXItem(pTVI->hItem);
	ASSERT(xti);
	...
	if(pTVI->mask & TVIF_TEXT)
	{
		...
		{
			uMaskCb |= TVIF_TEXT;
//			pTVI->pszText = xti->m_tvi.pszText;
		}
...
void COXTreeCtrl::SendGetDispInfoNotify(TV_ITEM *pTVI)
{
	TV_DISPINFO di;
...
CString COXTreeCtrl::GetItemText(HTREEITEM hItem,int nCol)
{
...
UINT COXTreeCtrl::GetVisibleCount()
{
	return CListCtrl::GetItemCount();
}

UINT COXTreeCtrl::GetCount()
{
...
HTREEITEM COXTreeCtrl::GetNextItem( HTREEITEM hItem, UINT nCode )
{
	...
	case TVGN_NEXTVISIBLE:
		return GetNextVisibleItem(hItem,TRUE);

	case TVGN_PREVIOUSVISIBLE:
		return GetPrevVisibleItem(hItem,TRUE);
...
{
	return GetNextItem(TVI_ROOT,TVGN_CARET);
}
...
BOOL COXTreeCtrl::ItemHasChildren( HTREEITEM hItem )
{
	COXTreeItem *xti = GetXItem(hItem);
	if(!xti)
		return FALSE;
...
HTREEITEM COXTreeCtrl::GetRootItem( )
{
	return (HTREEITEM)(&m_xtiRoot);
}

UINT COXTreeCtrl::GetItemState( HTREEITEM hItem, UINT nStateMask )
{
	TV_ITEM tvi;
	memset(&tvi,0,sizeof(tvi));
...
DWORD COXTreeCtrl::GetItemData( HTREEITEM hItem )
{
	TV_ITEM tvi;
	memset(&tvi,0,sizeof(tvi));
...
BOOL COXTreeCtrl::GetItemRect( HTREEITEM hItem, LPRECT lpRect, BOOL bTextOnly )
{
	COXTreeItem *xti = GetXItem(hItem);
	if(!xti)
		return FALSE;
...
COLORREF COXTreeCtrl::GetItemColor(HTREEITEM hItem,int nCol)
{
	COXTreeItem *xti = GetXItem(hItem);
	if(!xti)
		return 0;
	...
	case OXET_COMBO:
		{
			...
			if(xti->GetItemImg(nCol) != -1 &&pImgList)
			{
				IMAGEINFO ii;
...
BOOL COXTreeCtrl::IsItemHidden(HTREEITEM hItem)
{
	COXTreeItem *xti = GetXItem(hItem);
	if(!xti)
		return FALSE;
...
BOOL COXTreeCtrl::IsItemDisabled(HTREEITEM hItem)
{
	COXTreeItem *xti = GetXItem(hItem);
	if(!xti)
		return FALSE;
...
BOOL COXTreeCtrl::GetPickAnywhere()
{
	return (m_dwExStyle & TVOXS_PICKANYWHERE);
}

BOOL COXTreeCtrl::GetDrawGridFullLength()
{
	return (m_dwExStyle & TVOXS_FLGRID);
}

BOOL COXTreeCtrl::HasGrid(UINT nGridType)
{
...
UINT COXTreeCtrl::GetItemHeight()
{
...
HTREEITEM COXTreeCtrl::GetItemFromIndex(int nIndex)
{
	ASSERT(nIndex>=-1);
	if(nIndex==-1)
	{
						   ...
						   BOOL bCopyDescendants/*=TRUE*/)
{
	...
	pxParent->CopyChild(pxItemToCopy,pxInsertAfter,bCopyDescendants);
