								 ...
								 CString sCustomButtonText)
{
    ...
    m_sTearoffText=_T("Drag to make this menu float");
    m_sCaption=_T("Floating bar");

	// Define margines used to draw buttons. In your derived class you can redefine them.
	m_rectEdgeMargin=CRect(4,2,4,2);
	m_rectButtonMargin=CRect(0,0,0,0);
	m_rectDefaultButtonMargin=CRect(0,0,0,2);
	m_rectCustomButtonMargin=CRect(0,2,0,0);
    m_rectTearoffBarMargin=CRect(2,-1,2,2);
	...
	m_pTrackingDC=NULL;
	m_ptLastTrack=CPoint(0,0);
	m_rectTrackBar=CRect(0,0,0,0);
	m_rectLastTrackBar=CRect(0,0,0,0);
...
BOOL COXPopupBarCtrl::Create(CWnd* pParentWnd, UINT nPopupBarStyle, 
							 HBRUSH hbrBackground)
{
	...
	CRect rect(0,0,0,0);
    DWORD dwStyle=IsFloating() ? WS_VISIBLE|WS_SYSMENU : WS_POPUP; 
    DWORD dwExStyle=IsFloating() ? WS_EX_TOOLWINDOW : WS_EX_DLGMODALFRAME;
...
BOOL COXPopupBarCtrl::Pick(UINT nAlignment, CRect* pParentRect, CSize sizeOffset)
{
	...
	while(m_nState==0)
	{
		...
		{
			OnSysKeyUp(msg.wParam,LOWORD(msg.lParam),HIWORD(msg.lParam));
			continue;
		}

		if(msg.message==WM_SYSKEYDOWN)
		{
			OnSysKeyDown(msg.wParam,LOWORD(msg.lParam),HIWORD(msg.lParam));
			continue;
		}
...
BOOL COXPopupBarCtrl::SetButtonDataTable(ButtonDataTable& arrData)
{
	...
	UINT nElements=arrData.GetSize();
...
{
	UINT nElements=arrToolTipText.GetCount();
...
BOOL COXPopupBarCtrl::PopulateToolTip()
{
	...
	for(int nIndex=0; nIndex<nTools; nIndex++)
	{
...
{
	ASSERT(m_nButtons>0 && m_nButtons<=ID_POPUPBAR_MAX_BUTTONS);
	UINT nElements=m_arrData.GetSize();
...
{
	ASSERT(m_nButtons>0 && m_nButtons<=ID_POPUPBAR_MAX_BUTTONS);
	UINT nElements=m_arrButtonRect.GetSize();
	ASSERT(m_nButtons==nElements);
