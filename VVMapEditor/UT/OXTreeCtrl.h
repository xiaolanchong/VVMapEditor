...
public:
	BOOL Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwExStyle);
	...
	// --- Returns :
	//--- Effect : 
	inline int GetItemIndent(const HTREEITEM hItem)
	...
	BOOL GetItemDrawEllipsis( HTREEITEM hItem, UINT& nEllipsisFormat, int nCol=0 );
	// --- In  :	hItem			-	tree item handle
	//				nCol			-	index of column
	...
	HTREEITEM GetItemFromIndex(int nIndex);
	...
	//				hItemLast	-	handle of the item which children must be deleted
	// --- Out : 
	// --- Returns: Nonzero if successful; otherwise 0.
...
protected:
	...
	CWnd* StartEditSubItem(HWND hWndEdit,COXTreeItem *xitem,CRect& rcEdit,int nCol,LRESULT* pResult);
	int GetEditColumn(COXTreeItem *xItem,CRect *rcColText);
	int GetFullColumnWidth(int nCol);
	void SendGetDispInfoNotify(TV_ITEM *pTVI);
	void SendSetDispInfoNotify(TV_ITEM* pTVI);
	void SendDeleteItemNotify(TV_ITEM* ptvi);
	void DrawItemImages(CDC *pDC,COXTreeItem* xti,int nItem,CRect& rcItem,int nCol,BOOL bRowSel);
	CSize GetColTextExtent(int nIndex,int nCol=0);
	void GetItemPartRect(COXTreeCtrl::eItemPartRect eType,COXTreeItem *pItem,CRect *rTarget,int *nIndent);
	int GetItemIndent(COXTreeItem *pItem);
	BOOL Init();
	...
	int GetItemStateIndex(int nItem);
	...
	int GetItemIndexInternal(COXTreeItem *pItem);
