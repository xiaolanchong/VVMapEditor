...
int CGuiTabbed::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CDllResource DllRes;
	if (!m_img.Attach( DllRes.LoadImageList( IDB_DOCKBAR,9,11,RGB(255,0,255))) )
	{
		TRACE0("error imagelist");
	}
	CDllResource DllResources;
	...
	m_toolBtn[0].SetToolTip(DllResources.LoadString( IDS_SCROLL_LEFT ));
	...
	m_toolBtn[1].SetToolTip(DllResources.LoadString( IDS_SCROLL_RIGHT ));
	...
	m_toolBtnC.SetToolTip(DllResources.LoadString( IDS_CLOSE ));
...
void CGuiTabbed::EnableClose(BOOL bEnable /* = TRUE */)
{
  m_toolBtnC.EnableWindow(bEnable);
}
