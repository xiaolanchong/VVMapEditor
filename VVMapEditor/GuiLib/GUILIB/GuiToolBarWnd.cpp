...
//NOTENOTE: 2004-09-10 Eugene V. Gorbachev revision, fixed SetButtonDropDown
...
public:
	...
	CDropmenu(UINT nID,HMENU hMenu)
	{
		nIDboton=nID;
		nMenu = 0;
		m_pWnd=NULL;
...
{
	m_style = Office2000;  //by default
	m_pArray.RemoveAll();
	//NOTENOTE: This is cahce value = CArray::GetSize, i didn't delete if for binary compatible;
	m_iElements=0;
	m_bIsXp=TRUE;
	bPressDown=FALSE;
	CDllResource DllRes;
	m_imgArrow.Attach(  DllRes.LoadImageList( IDB_DOCKBAR,9,10,RGB(255,0,255) ));
...
void CGuiToolBarWnd::SetButtonDropDown(UINT nID,int iImage,UINT nMENU)
{
	...
	//NOTENOTE: ensure array doesnt have our menu
	for( int i=0; i < m_pArray.GetSize(); ++i)
	{
		CDropmenu* pDrop = static_cast<CDropmenu*>(m_pArray[i]);
		if( pDrop->nIDboton == nID )
		{
			pDrop->m_pWnd	= 0;
			pDrop->nMenu	= nMENU;
			pDrop->m_hMenu	= 0;
	...
	//NOTENOTE: not in array, just add
	m_pArray.Add(new CDropmenu(nID,nMENU));
//	m_iElements++;
}

//-----------------------------------------------------------------------------
// Purpose: Load dynamic updating menu from handle & attach it to toolbar button with
//			arrow, see changes in CDropdown
// Input  : nID - resource id of toolbar button
//			iImage - image index in imagelist
//			hMenu - menu handle
//-----------------------------------------------------------------------------
void CGuiToolBarWnd::SetButtonDropDown(UINT nID,int iImage, HMENU hMenu)
{
	CToolBarCtrl &m_tbc= GetToolBarCtrl();
	m_tbc.SetExtendedStyle(TBSTYLE_EX_DRAWDDARROWS);
	m_tbc.SetStyle(m_tbc.GetStyle() | TBSTYLE_LIST | CCS_ADJUSTABLE);
	...
	m_pArray.Add(new CDropmenu(nID,hMenu));
//	m_iElements++;
}
...
void CGuiToolBarWnd::SetButtonDropDown(UINT nID,int iImage,CWnd* pWnd,LPCTSTR lpTitle)
{
	...
	m_pArray.Add(new CDropmenu(nID,pWnd,lpTitle));
//	m_iElements++;
}
...
BOOL CGuiToolBarWnd::OnDropDown(NMHDR* pNMHDR, LRESULT* pRes)
{
	...
	else if( ((CDropmenu*)m_pArray[i])->m_hMenu > 0 )
	{
		bPressDown=TRUE;
		CMenu m_menu;
		m_menu.Attach(((CDropmenu*)m_pArray[i])->m_hMenu);
		gbintHorz=0;			//Horz=0, Vert=1
		rcMenu=rc;					//CRect of button
		m_menu.TrackPopupMenu(TPM_LEFTALIGN|TPM_LEFTBUTTON|TPM_VERTICAL,
				rc.left, rc.bottom-2, AfxGetMainWnd(), &rc);
		m_menu.Detach();
		bPressDown=FALSE;
	}
...
#ifdef TB_ARROW
  CRect rcArrowBtn ( CPoint( rcClient.right - m_nArrowWidth, rcClient.top),
						CSize( m_nArrowWidth, rcClient.Height()));
	DrawArrow(&dc, rcArrowBtn);
#endif
//	dc.FillSolidRect( pt.x, pt.y, 6, 6, RGB(0,0,0) );	
	...
	for(int i=0; i < nCountBtn; i++)
	{
		...
		{
			if( ! CRect().IntersectRect(rcAreaUpdate, rcBtn) ) continue;
			...
			if (mbIsEnabled)
			{
				...
				{
					CRect rz(rcBtn);
					rz.DeflateRect(0, 0, 1, 0);
					
					if (wState & TBSTATE_CHECKED)
					{
						dc.Draw3dRect(rz,GuiDrawLayer::GetRGBCaptionXP(),GuiDrawLayer::GetRGBCaptionXP());
						rz.DeflateRect(1,1);
						cblu.CreateSolidBrush(GuiDrawLayer::GetRGBLightFondoXP());
						dc.FillRect(rz,&cblu);					
					}
					else
					{
		...
		//			rcBtn.DeflateRect(0,0,1,0);
					if (wState & TBSTATE_CHECKED)
					{
						CRect rz(rcBtn);
						rz.DeflateRect(0, 0, 1, 0);

						cblu.CreateSolidBrush(GuiDrawLayer::GetRGBDarkFondoXP());
						dc.FillRect(rz,&cblu);					
					}
					else
          {
...
#ifdef TB_ARROW
		if( dwMode & LM_HORZ )
		sizeResult.cx += m_nArrowWidth;
		else sizeResult.cy += m_nArrowWidth;
#endif
		if (dwMode & LM_COMMIT)
		{
