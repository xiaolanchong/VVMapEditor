...
#ifndef _WIN32
    UINT temp = ::GetWindowWord(pParentActual->m_hWnd, GWW_ID);
    ::SetWindowWord(pParentActual->m_hWnd, GWW_ID, ::GetWindowWord(pParent->m_hWnd, GWW_ID));
    ::SetWindowWord(pParent->m_hWnd, GWW_ID, temp);
#else
    UINT temp = ::GetWindowLong(pParentActual->m_hWnd, GWL_ID);
    ::SetWindowLong(pParentActual->m_hWnd, GWL_ID, ::GetWindowLong(pParent->m_hWnd, GWL_ID));
    ::SetWindowLong(pParent->m_hWnd, GWL_ID, temp);
#endif
  }

  if (m_alnTab!=ALN_TOP || TYPE_SDI == m_TypeTab)
	{
		if (m_Numtabs==0)
...
void CGuiTabWnd::SetCurtab(int m_numtab)
{
    ...
    if (TYPE_SDI == m_TypeTab)
    {
      pParentActual->ShowWindow(SW_HIDE);

      // Exchange view window ID's so RecalcLayout() works.
      UINT temp = ::GetWindowLong(pParentActual->m_hWnd, GWL_ID);
      ::SetWindowLong(pParentActual->m_hWnd, GWL_ID, ::GetWindowLong(pParentNew->m_hWnd, GWL_ID));
      ::SetWindowLong(pParentNew->m_hWnd, GWL_ID, temp);

      if (pParentNew->IsKindOf(RUNTIME_CLASS(CView)))
        GetParentFrame()->SetActiveView((CView*)pParentNew);
      else
    ...
    // send TCN_SELCHANGE to parent window
    NMHDR nmhdr;
    nmhdr.hwndFrom = GetSafeHwnd();
    nmhdr.idFrom = _AfxGetDlgCtrlID(nmhdr.hwndFrom);
