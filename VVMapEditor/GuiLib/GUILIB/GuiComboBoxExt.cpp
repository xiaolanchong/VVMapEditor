...
CGuiComboBoxExt::CGuiComboBoxExt()
{
	...
	m_bPress=FALSE;
	//NOTENOTE: 18.10.2004 to explicit res loading
...
void CGuiComboBoxExt::SaveHistory(CString szHistory,BOOL bSaveCurrent)
{
	...
	if(bSaveCurrent)
	{
		...
		CString nKeyHist=szHistory+_T("SaveCurrent");
...
void CGuiComboBoxExt::DeleteHistory(CString szHistory)
{
    ...
    if (pAppWin->m_pszRegistryKey == NULL || pAppWin->m_pszAppName == NULL)
      return;
    CString sPath = _T("SOFTWARE\\");
	sPath+= pAppWin->m_pszRegistryKey + CString(_T("\\"));
    sPath+= pAppWin->m_pszAppName + CString(_T("\\"));
    sPath+= szHistory;
