...
//
void CCoolMenuManager::Install(CFrameWnd* pFrame, CWnd* pHookWnd)
{
	ASSERT_VALID(pFrame);
	m_pFrame = pFrame;
	HookWindow(pHookWnd);   // install message hook
}
...
BOOL CCoolMenuManager::LoadToolbar(UINT nIDToolbar)
{
	...
	HINSTANCE hInst= AfxFindResourceHandle(lpResName, RT_TOOLBAR);
	return LoadToolbar( hInst, nIDToolbar);
}

BOOL	CCoolMenuManager::LoadToolbar( HINSTANCE hInst, UINT nIDToolbar) 
{
/*	 In MFC this structure use for loading toolbars' bitmaps, we need RGB(192,192,192)
*/
	LPTSTR lpResName = MAKEINTRESOURCE(nIDToolbar);
	// load bitmap
...
//	HINSTANCE hInst;
	...
	COLORREF z = GetSysColor(COLOR_3DSHADOW);
	m_ilButtons.Add(&bmToolbar,/*GuiDrawLayer::GetRGBColorFace()*/RGB(0xC0, 0xC0, 0xC0));
		...
		if (true) {
			...
			if (!bDisabled) {
				...
				if (bChecked )
				{
					...
					rcCheck=rcButn;
					rcCheck.InflateRect(0, 0, 2, 1);
					PLSelectRect(dc, rcCheck);
					CBrush brush;
					COLORREF cl = bSelected? GuiDrawLayer::GetRGBDarkFondoXP() : GuiDrawLayer::GetRGBFondoXP();
					brush.CreateSolidBrush(cl);
					dc.FillRect(rcCheck,&brush);
					dc.Draw3dRect(rcCheck,GuiDrawLayer::GetRGBCaptionXP(),GuiDrawLayer::GetRGBCaptionXP()); 
				}
				// draw pushed-in or popped-out edge
				if (bSelected || bChecked) {
					rc2 = rcButn;
		//NOTENOTE: когда имеется картинка в image list для пункта меню, галочку рисовать не надо
					if (bChecked && !m_ilButtons.ExtractIcon(iButton))
					{
						rc2.DeflateRect(5,5);
				...
				// use DrawEmbossed to draw disabeld button, w/color flag GuiDrawLayer::GetRGBColorShadow()
			{
				HICON hIcon = m_ilButtons.ExtractIcon(iButton);
				dc.DrawState(p,m_szButton,hIcon,DSS_DISABLED, &CBrush (GuiDrawLayer::GetRGBColorShadow()));
				DestroyIcon(hIcon);
	...
	for (UINT i = 0; i < nItem; i++) {	
		...
		if (bShowButtons) {
			...
			if (info.fMask & MIIM_TYPE) {
				...
				_tcsncpy(itemname, sItemName, sizeof(itemname)/sizeof(TCHAR));
				info.dwTypeData = itemname;
...
CString CCoolMenuManager::GetVirtualKeyName( const CString strVirtKey ) const
{
	...
	if		( strVirtKey == DllRes.LoadString( IDS_KEY_NUM_7 ))		{ strResult = DllRes.LoadString( IDS_KEY_HOME	); }
	else if ( strVirtKey == DllRes.LoadString( IDS_NEY_NUM_1 ))		{ strResult = DllRes.LoadString( IDS_KEY_END	); }
	else if ( strVirtKey == DllRes.LoadString( IDS_KEY_NUM_9 ))		{ strResult = DllRes.LoadString( IDS_KEY_PGDN	); }
	else if ( strVirtKey == DllRes.LoadString( IDS_KEY_NUM_3 ))		{ strResult = DllRes.LoadString( IDS_KEY_PGUP	); }
	else if ( strVirtKey == DllRes.LoadString( IDS_KEY_NUM_0 ) )	{ strResult = DllRes.LoadString( IDS_KEY_INS	); }
	else if ( strVirtKey == DllRes.LoadString( IDS_KEY_NUM_DEL ))	{ strResult = DllRes.LoadString( IDS_KEY_DEL	); }
	else
	{
...
// HEUZ 
// HACK!!!!!!!!!!!!!!!!!
// Since VK_PAUSE & VK_CANCEL has no scan code translation( return 0) 
// we do it localy
static CString GetKeyNameFromVK( UINT vkey )
{
	CDllResource DllRes;
	TCHAR keyname[64];
	switch(vkey)
	{
	case VK_PAUSE  : return DllRes.LoadString( IDS_KEY_BREAK );
	case VK_CANCEL : return DllRes.LoadString( IDS_KEY_CTRL_BREAK );
	default:
		{	UINT ScanCode = MapVirtualKey(vkey, 0)<<16;
...
BOOL CCoolMenuManager::AppendAccelName(CString& sItemName, UINT nID)
{
	...
	for (ACCEL* pa = GetAccel(nID); pa; pa -= pa->cmd) {
		...
		if (pa->fVirt & FALT)			sItemName += DllRes.LoadString( IDS_KEY_ALT_MODIFIER );
		if (pa->fVirt & FCONTROL)		sItemName += DllRes.LoadString( IDS_KEY_CTRL_MODIFIER );
		if (pa->fVirt & FSHIFT)			sItemName += DllRes.LoadString( IDS_KEY_SHIFT_MODIFIER );
		if (pa->fVirt & FVIRTKEY) {
			
			CString keyname = GetKeyNameFromVK( pa->key );
...
/*	HINSTANCE hInst = AfxFindResourceHandle(lpResName, RT_BITMAP);
	...
	// HEUZ 
	return LoadBitmap( AfxFindResourceHandle(lpResName, RT_BITMAP), lpResName );
	// HEUZ
}
