...
CGuiControlBar::CGuiControlBar()
{
	...
	m_bActive=FALSE;
	m_bOldActive=FALSE;
	m_sizeMinFloating=m_sizeVert=m_sizeHorz=CSize(200,163);
	m_sizeHorzt=CSize(200,163);
	m_sizeVertt=CSize(200,163);
	m_pos=0;
	m_Last=0;
	m_bTracking=FALSE;
...
int CGuiControlBar::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_MenuManager.Install( GetParentFrame(), this);
	m_img.Create( 16, 15, ILC_COLOR24|ILC_MASK, 0, 10 );

	if (m_pDockContext==NULL)
		m_pDockContext=new CGuiDockContext(this);
	ASSERT(m_pDockContext);
	CDllResource DllRes;
	m_CloseBtn.SetData(6, DllRes.LoadString( IDS_CLOSE ));
	//NOTENOTE: from internal resources
	m_CloseBtn.SetImageList( DllRes.GetHandle(), IDB_DOCKBAR,9,10,RGB(255,0,255));
	return 0;
}
...
CSize CGuiControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
	...
	if (IsFloating() || m_bKeepSizeWhenDocked)
...
{
  if (m_bKeepSizeWhenDocked)
  {
    if (nMode & (LM_VERTDOCK|LM_HORZDOCK))
    {
      nMode &= ~(LM_HORZDOCK|LM_VERTDOCK);
      nMode |= LM_MRUWIDTH;
    }
  }
	if (IsFloating())
	...
	if(nMode & LM_LENGTHY)
  {
    ...
    m_sizeVert = m_sizeHorz = m_sizeMinFloating;    
  }
	else
  {
    m_sizeMinFloating.cx = max(nLength,32);
    m_sizeHorz = m_sizeVert = m_sizeMinFloating;
  }
	return m_sizeMinFloating;
}
...
void CGuiControlBar::OnNcRButtonDown(UINT nHitTest, CPoint point)
{
	...
	m_MenuManager.LoadToolbar( DllResource.GetHandle(), IDR_CONTROLBAR);
	...
	if ((dwDockStyle != 0 || m_dwAllowDockingState != 0))	// ie docking is actually allowed ...
	{
		...
		strMenu = DllResource.LoadString(ID_CB_ALLOW_DOCKING);
		VERIFY( !strMenu.IsEmpty());
		m_SysMenu.AppendMenu(style, ID_CB_ALLOW_DOCKING, strMenu);
	}
	strMenu = DllResource.LoadString( ID_CB_HIDE );
	VERIFY(!strMenu.IsEmpty());
	m_SysMenu.AppendMenu(MF_STRING, ID_CB_HIDE, strMenu);
...
void CGuiControlBar::OnNcCalcSize(BOOL /*bCalcValidRects*/, NCCALCSIZE_PARAMS* lpncsp)
{
	...
	lpncsp->rgrc[0].right -=/*!IsFloating()?*/5/*:2*/;
...
//        return CControlBar::OnNcHitTest(point);
	...
	if (rcT.PtInRect(pt) && (GetStyle() & WS_SYSMENU))
		return HTCLOSE;
	CRect rcTemp;
	for (int i=0; i< 4; i++)
	{
...
{
	// TODO: Add your message handler code here and/or call default
	if (IsFloating()) return CControlBar::OnSetCursor(pWnd, nHitTest, message);
	CPoint ptCursor;
...
void CGuiControlBar::OnNcMouseMove(UINT nHitTest, CPoint point)
{
	...
	if (nHitTest == HTCLOSE /*&& !IsFloating()*/)
	{
		m_stateBtn=OVER;
		SetTimer(1,100,0);
	}
...
void CGuiControlBar::OnAllowDocking()
{
...
void CGuiControlBar::OnHide()
{
	CFrameWnd* pParentFrameWnd = GetParentFrame();
	pParentFrameWnd->ShowControlBar(this, FALSE, FALSE);	
}

void CGuiControlBar::OnCloseBtn()
{
	GetDockingFrame()->ShowControlBar(this, FALSE, FALSE);
	GetParentFrame()->RecalcLayout();
}

void CGuiControlBar::SetWindowText(LPCTSTR lpString)
{
  CControlBar::SetWindowText(lpString);
  SendMessage(WM_NCPAINT);
}
