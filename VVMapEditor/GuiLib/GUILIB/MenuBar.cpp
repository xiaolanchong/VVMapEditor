...
//NOTENOTE: 10/13/2004 now in resources
...
void CMenuBar::OnInitMenuPopup(CMenu* pPopupMenu, UINT nIndex, BOOL bSysMenu)
{
	...
	for (iWin = 1; iWin <= 10; iWin++, nID++)
	{
		...
		{
			CString _szMoreWindows = CDllResource().LoadString( IDS_MORE_WINDOWS );
...
CMenuControl::CMenuControl(CWnd* pMenuBar)
{
	...
	m_arrButton[2].SetToolTip( DllResource.LoadString( IDS_CLOSE )  );
	...
	m_arrButton[1].SetToolTip(DllResource.LoadString( IDS_RESTORE ));
	...
	m_arrButton[0].SetToolTip(DllResource.LoadString( IDS_MINIMIZE ));
