...
_inline void ASSERT_BOOL(BOOL x) 
{ 
	if(!x) __asm int 3;
}
#else
...
public:
		...
		MemStream(IMalloc* pAlloc,  UINT uMemoryBlockSize, PBYTE pBuffer, UINT cbSize, BOOL bReadOnly );
private:
...
MemStream::MemStream(	IMalloc* pAlloc, 
						UINT uMemoryBlockSize, 
						PBYTE pBuffer, UINT cbSize,
						BOOL	bReadOnly):
	m_pAlloc(pAlloc),
	...
	m_cbData(cbSize),
	m_iSeek(0),
	m_uMemoryBlockSize(uMemoryBlockSize),
...
STDMETHODIMP MemStream::Write(void const *pv, ULONG cb, ULONG *pcbWritten)
{
    ...
    if ((this->m_iSeek + cb) > this->m_cbAlloc)
    {
        ...
        if (GrowBuffer(this->m_iSeek + (UINT)cb + /*SIZEINCR*/m_uMemoryBlockSize) == NULL)
            return STG_E_INSUFFICIENTMEMORY;
    }

	if(m_bReadOnly) return  STG_E_ACCESSDENIED;
...
IMemStream*	CreateStreamOnMalloc( IMalloc* pAlloc, UINT  uMemoryBlock, 
								  BYTE* pBuffer, UINT uSize, BOOL bReadOnly )
{
#if defined _DEBUG && !defined _WIN32_WCE
		_CrtSetDbgFlag( _CRTDBG_LEAK_CHECK_DF | _CRTDBG_ALLOC_MEM_DF );
#endif	
	if( ! pAlloc ) return 0;
	if( !uMemoryBlock ) return 0;
	return ::new MemStream( pAlloc, uMemoryBlock, pBuffer, uSize, bReadOnly );
}
